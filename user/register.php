...
/**
 * @file register.php
 * @description 用户注册页，含邮箱验证与注册开关控制
 * @author AI
 * @version 1.1.0-dev
 * @date 2026-08-04
 */
...
$emailVerifyReq = is_email_verify_required();
...
<title>用户注册 - <?php echo htmlspecialchars($siteName);?></title>
...
.orb.o1{width:420px;height:420px;background:radial-gradient(circle,rgba(123,140,255,.45),transparent 70%);top:-120px;right:-100px}
...
.register-wrap{position:relative;z-index:1;width:440px;max-width:92%;padding:0 10px}
.register-card{
...
.register-title{text-align:center;font-family:var(--font-display);font-size:24px;font-weight:800;margin:22px 0 4px;letter-spacing:-.5px}
.register-subtitle{text-align:center;font-size:13px;color:var(--text-muted);margin-bottom:26px}
...
.input-wrap:focus-within .mdi{color:var(--primary)}
...
.btn-send-code{
  flex-shrink:0;padding:13px 18px;border-radius:var(--radius-sm);border:1.5px solid var(--primary);
  background:var(--surface-2);color:var(--primary);font-size:13px;font-weight:600;font-family:var(--font-body);
  cursor:pointer;transition:all .2s var(--ease-out);white-space:nowrap;display:flex;align-items:center;gap:6px;
}
.btn-send-code:hover:not(:disabled){background:var(--primary);color:#fff}
.btn-send-code:disabled{opacity:.5;cursor:not-allowed}

/* 密码强度 */
.pwd-strength{height:4px;border-radius:999px;background:var(--border-strong);margin-top:8px;overflow:hidden}
...
.register-footer{margin-top:22px;text-align:center;font-size:13px;color:var(--text-muted)}
.register-footer a{color:var(--primary);font-weight:600;text-decoration:none;transition:opacity .2s}
.register-footer a:hover{opacity:.8;text-decoration:underline}
.back-home{display:flex;align-items:center;justify-content:center;gap:6px;margin-top:14px;padding:10px;border-radius:var(--radius-sm);border:1px solid var(--border);background:var(--surface-2);color:var(--text-2);font-size:13px;font-weight:600;text-decoration:none;transition:all .2s}
...
.reg-closed .icon{font-size:56px;color:var(--text-light);margin-bottom:16px}
.reg-closed .title{font-family:var(--font-display);font-size:20px;font-weight:700;margin-bottom:8px}
.reg-closed .desc{font-size:13.5px;color:var(--text-muted);line-height:1.6}
...
<div class="register-wrap">
  <div class="register-card">
    ...
    <div class="reg-closed">
      <div class="icon"><i class="mdi mdi-account-lock-outline"></i></div>
      <div class="title">注册已关闭</div>
      <div class="desc">当前站点已关闭新用户注册功能，如需账号请联系管理员。</div>
    </div>
    <a href="login.php" class="back-home">
      <i class="mdi mdi-arrow-left"></i> 返回登录
    </a>
    <?php else: ?>
    <h1 class="register-title">创建账号</h1>
    <p class="register-subtitle">注册即可享受图床上传与管理服务</p>
    ...
    <form id="registerForm" method="post" autocomplete="off">
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          <input type="text" class="form-input" id="username" name="username" placeholder="3-32位，支持字母数字下划线中文" required autofocus>
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          <input type="password" class="form-input" id="password" name="password" placeholder="6-64位，需包含字母和数字" required>
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          <input type="password" class="form-input" id="confirmPassword" name="confirm_password" placeholder="请再次输入密码" required>
          <i class="mdi mdi-lock-check-outline"></i>
        </div>
      </div>
      <?php if($emailVerifyReq): ?>
      <div class="form-group">
        ...
        <div class="form-hint">用于接收注册验证码，请填写有效邮箱</div>
      </div>
      <div class="form-group">
        <label class="form-label" for="verifyCode">邮箱验证码</label>
        <div class="code-row">
          <div class="input-wrap">
            <input type="text" class="form-input" id="verifyCode" name="verify_code" placeholder="请输入 6 位验证码" maxlength="6" required>
            <i class="mdi mdi-shield-key-outline"></i>
          </div>
          <button type="button" class="btn-send-code" id="btnSendCode">
            <i class="mdi mdi-email-send-outline"></i> <span id="sendCodeText">发送验证码</span>
          </button>
        </div>
        <div class="form-hint" id="codeHint">验证码 10 分钟内有效</div>
      </div>
      <?php endif; ?>
      <button type="submit" class="btn-submit" id="submitBtn">
        <i class="mdi mdi-account-plus"></i> <span>注 册</span>
      </button>
    </form>

    <div class="register-footer">
      已有账号？ <a href="login.php">返回登录 &rarr;</a>
    </div>
...
var emailVerifyReq = <?php echo $emailVerifyReq ? 'true' : 'false';?>;
...
document.getElementById('password').addEventListener('input', function(){
  ...
  var widths = [0, 25, 45, 65, 85, 100];
  var colors = ['transparent', '#ff4757', '#ff4757', '#ffa600', '#7dd3fc', '#00c896'];
...
var sendCodeCountdown = 60;
var btnSendCodeEl = document.getElementById('btnSendCode');
if(btnSendCodeEl) btnSendCodeEl.addEventListener('click', function(){
  var email = document.getElementById('email').value.trim();
  if(!email){
    notify('请先输入邮箱地址', 'warning');
    return;
  }
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){
  ...
  var btn = this;
  var btnText = document.getElementById('sendCodeText');
  btn.disabled = true;
  ...
  fetch('../api/user_api.php?action=send_code', {method:'POST', body:fd})
    .then(function(r){ return r.json(); })
    .then(function(data){
      if(data.code === 0){
        notify(data.msg || '验证码已发送', 'success');
        // 开始倒计时
        sendCodeCountdown = 60;
        btnText.textContent = sendCodeCountdown + 's 后重发';
...
document.getElementById('registerForm').addEventListener('submit', function(e){
  ...
  var email = emailVerifyReq ? document.getElementById('email').value.trim() : '';
  var verifyCode = emailVerifyReq ? document.getElementById('verifyCode').value.trim() : '';

  if(username.length < 3 || username.length > 32){
    notify('用户名长度需要 3-32 个字符', 'warning'); return;
  }
  if(password.length < 6 || password.length > 64){
    notify('密码长度需要 6-64 位字符', 'warning'); return;
  }
  // M7: 前端密码复杂度校验
  if(!/[a-zA-Z]/.test(password) || !/[0-9]/.test(password)){
    notify('密码必须包含字母和数字', 'warning'); return;
  }
  if(password !== confirmPassword){
    notify('两次输入的密码不一致', 'warning'); return;
  }
  if(emailVerifyReq && !email){
    notify('请输入邮箱地址', 'warning'); return;
  }
  if(emailVerifyReq && !verifyCode){
    notify('请输入邮箱验证码', 'warning'); return;
  }

  var btn = document.getElementById('submitBtn');
  var origHTML = btn.innerHTML;
  btn.disabled = true;
  btn.innerHTML = '<i class="mdi mdi-loading mdi-spin"></i> <span>注册中...</span>';
  ...
  if(emailVerifyReq){
    fd.append('email', email);
    fd.append('verify_code', verifyCode);
  }
  fd.append('csrf_token', csrfToken);

  fetch('../api/user_api.php?action=register', {method:'POST', body:fd})
    .then(function(r){ return r.json(); })
    .then(function(data){
      if(data.code === 0){
        notify(data.msg || '注册成功，即将跳转到登录页面', 'success');
        setTimeout(function(){ window.location.href = 'login.php'; }, 1500);
      } else {
        notify(data.msg || '注册失败', 'error');
        btn.disabled = false;
