...
// ========== 悠悠图床：基于兰空图床(Lsky Pro)，无需登录、无需Token、无需Cookie ==========
...
if ($_FILES["file"]["size"] > 4997 * 1024 * 1024) {
    echo json_encode(["code" => 201, "msg" => "文件大小超出限制！最大只能上传4.88GB的文件！"]); exit;
}
...
curl_setopt_array($ch, [
    CURLOPT_URL => 'https://uuimg.com/api/v2/upload',
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => [
    ...
    CURLOPT_HTTPHEADER => [
        'Origin: https://uuimg.com',
        'Referer: https://uuimg.com/',
...
if ($resp === false || $resp === '') {
    echo json_encode(["code" => 201, "msg" => "连接悠悠图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode(["code" => 201, "msg" => "悠悠图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : "")], JSON_UNESCAPED_UNICODE); exit;
}

$status = $imgData['status'] ?? '';
$imgUrl = $imgData['data']['public_url'] ?? '';

if ($httpCode === 200 && $status === 'success' && $imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
    echo json_encode(["code" => 200, "msg" => "上传成功！", "path" => $imgUrl], JSON_UNESCAPED_UNICODE);
} elseif ($httpCode === 403) {
    echo json_encode(["code" => 201, "msg" => "悠悠图床禁止访问（403），Origin/Referer校验失败。"]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过悠悠图床限制。"]);
} else {
