...
if ($_FILES["file"]["size"] > 3 * 1024 * 1024) {
    echo json_encode(["code" => 201, "msg" => "文件大小超出限制！最大只能上传约2.91MB的文件！"]); exit;
}
...
$endpoint = 'https://tucdn.wpon.cn/upload/upload.html';
...
curl_setopt_array($ch, [
    ...
    CURLOPT_HTTPHEADER => [
        'Origin: https://tucdn.wpon.cn',
        'Referer: https://tucdn.wpon.cn/',
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接极速图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode([
        "code" => 201,
        "msg" => "极速图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : ""),
...
if ($code === 200 && $imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
    ...
    echo json_encode(["code" => 201, "msg" => "极速图床请求错误（400）：" . $errorMsg]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过极速图床限制。"]);
} else {
