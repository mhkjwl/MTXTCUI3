<?php
/**
 * @file api/naixiai.php
 * @description 奶昔图床上传适配器
 * @author AI
 * @version 1.1.0-dev
 * @date 2026-08-04
 */
...
// 检查API Key是否已配置
$apiKey = trim($conf['api_naixiai_key'] ?? '');
if (empty($apiKey)) {
    echo json_encode(["code" => 201, "msg" => "请先在后台API接口设置中填写奶昔图床 API Key 后再上传图片！"]); exit;
}
...
$endpoint = 'https://naixiai.cn/api/1/upload';
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接奶昔图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode([
        "code" => 201,
        "msg" => "奶昔图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : ""),
...
if ($statusCode === 200) {
    ...
    if ($imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
        ...
        echo json_encode(["code" => 201, "msg" => "奶昔图床返回成功但URL无效", "debug" => $uploadimg], JSON_UNESCAPED_UNICODE);
    }
} elseif ($statusCode === 400) {
    $errorMsg = $imgData['error']['message'] ?? ($imgData['status_txt'] ?? '参数错误');
    // 重复上传：虽然 status_code 为 400，但图片已存在
    ...
    echo json_encode(["code" => 201, "msg" => "奶昔图床参数错误（400）：" . $errorMsg]);
} elseif ($statusCode === 401 || $statusCode === 403) {
    echo json_encode(["code" => 201, "msg" => "奶昔图床 API Key 鉴权失败，请检查后台配置的Key是否正确！"]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过奶昔图床限制，请尝试上传更小的文件。"]);
} else {
