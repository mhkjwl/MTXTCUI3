...
$sign = time() . '.' . random_int(1, 999);

$endpoint = 'https://matuimg.com/app/upload.php';
...
curl_setopt_array($ch, [
    ...
    CURLOPT_HTTPHEADER => [
        'Origin: https://matuimg.com',
        'Referer: https://matuimg.com/',
        'Accept: */*',
    ],
]);
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接宝马图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode(["code" => 201, "msg" => "宝马图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : "")], JSON_UNESCAPED_UNICODE); exit;
}
...
if ($result === 'success' && $imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
    ...
    echo json_encode(["code" => 201, "msg" => "宝马图床请求错误（400）：缺少必要字段。"]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过宝马图床限制。"]);
} else {
