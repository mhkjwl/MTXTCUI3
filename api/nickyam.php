<?php
/**
 * @file api/nickyam.php
 * @description Telegraph2图床上传适配器
 * @author AI
 * @version 1.1.0-dev
 * @date 2026-08-04
 */
...
$endpoint = 'https://img.nickyam.com/upload';
...
curl_setopt_array($ch, [
    ...
    CURLOPT_HTTPHEADER => [
        'Origin: https://img.nickyam.com',
        'Referer: https://img.nickyam.com/',
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接Telegraph2图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if ($src !== '') {
    $imgUrl = 'https://img.nickyam.com' . $src;
    if (filter_var($imgUrl, FILTER_VALIDATE_URL)) {
        echo json_encode(["code" => 200, "msg" => "上传成功！", "path" => $imgUrl], JSON_UNESCAPED_UNICODE);
    } else {
        echo json_encode(["code" => 201, "msg" => "Telegraph2图床返回的URL无效", "debug" => $uploadimg], JSON_UNESCAPED_UNICODE);
    }
} elseif ($httpCode === 400) {
    echo json_encode(["code" => 201, "msg" => "Telegraph2图床请求错误（400）：缺少 file 字段。"]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过Telegraph2图床限制（5MB）。"]);
} else {
    $snippet = mb_substr(trim(strip_tags($uploadimg)), 0, 300);
    echo json_encode(["code" => 201, "msg" => "上传失败（HTTP " . $httpCode . "）" . ($snippet ? "：" . $snippet : ""), "debug" => $uploadimg], JSON_UNESCAPED_UNICODE);
}
