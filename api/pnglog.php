...
// 盘络图床 API v2 上传接口（基于兰空图床，匿名上传）
$endpoint = 'https://www.pnglog.com/api/v2/upload';
...
curl_setopt_array($ch, [
    ...
    CURLOPT_HTTPHEADER => [
        'Origin: https://www.pnglog.com',
        'Referer: https://www.pnglog.com/',
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接盘络图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode([
        "code" => 201,
        "msg" => "盘络图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : ""),
...
if ($status && $imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
    ...
    echo json_encode(["code" => 201, "msg" => "盘络图床禁止访问（403），可能需要 Cloudflare cf_clearance Cookie。"]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过盘络图床限制（5MB）。"]);
} else {
