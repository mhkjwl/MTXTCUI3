...
/**
 * @file api/feimg.php
 * @description FE图床上传适配器（freeimage.host）
 * @author AI
 * @version 1.1.0-dev
 * @date 2026-08-04
 */
...
/**
 * FE图床 API（freeimage.host）
 * 文档：POST https://freeimage.host/api/1/upload
 * 字段：key（可选）、action=upload、source（文件）
 */
require ('../inc/lang.php');
require ('../inc/common.php');
header("Content-type:application/json");

// freeimage.host 必须携带 key，使用官网公开默认 key（用户无需配置）
...
if ($_FILES["file"]["size"] > 64 * 1024 * 1024) {
    echo json_encode(["code" => 201, "msg" => "文件大小超出限制！最大只能上传64MB的文件！"]); exit;
}
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接FE图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode([
        "code" => 201,
        "msg" => "FE图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : ""),
...
if ($statusCode === 200) {
    ...
    if ($imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
        ...
        echo json_encode(["code" => 201, "msg" => "FE图床返回成功但URL无效", "debug" => $uploadimg], JSON_UNESCAPED_UNICODE);
    }
} elseif ($statusCode === 400) {
    $errorMsg = $imgData['error']['message'] ?? ($imgData['status_txt'] ?? '参数错误');
    echo json_encode(["code" => 201, "msg" => "FE图床参数错误（400）：" . $errorMsg]);
} elseif ($statusCode === 401 || $statusCode === 403) {
    echo json_encode(["code" => 201, "msg" => "FE图床 API Key 鉴权失败，请检查后台配置的Key是否正确！"]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过FE图床限制（64MB）。"]);
} else {
