...
/**
 * @file api/yopngs.php
 * @description 有图床上传适配器（yopngs.com / Backblaze）
 * @author AI
 * @version 1.1.0-dev
 * @date 2026-08-04
 */
...
$endpoint = 'https://yopngs.com/api/upload?type=backblaze';

$ch = curl_init();
curl_setopt_array($ch, [
    CURLOPT_URL => $endpoint,
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => ['image' => new CURLFile($filepath, $fileType, random_name(12).'.'.$extension)],
    ...
    CURLOPT_HTTPHEADER => [
        'Origin: https://yopngs.com',
        'Referer: https://yopngs.com/',
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接有图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode([
        "code" => 201,
        "msg" => "有图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : ""),
...
if ($code === 200 && $imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
    ...
    echo json_encode(["code" => 201, "msg" => "有图床请求错误（400）：" . $errorMsg]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过有图床限制。"]);
} else {
