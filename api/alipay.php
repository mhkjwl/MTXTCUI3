...
$cookie = trim($conf['api_alipay_cookie'] ?? '');
if ($cookie === '') {
    echo json_encode(["code" => 201, "msg" => "请先在后台支付宝图床设置中填写Cookie（支付宝ALIPAYJSESSIONID值）后再上传图片！"]); exit;
}
...
curl_setopt_array($ch, [
    CURLOPT_URL => 'https://ffapi.cn/int/v1/image.alipay',
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => [
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接支付宝图床失败 [HTTP " . $httpCode . "]"]);
    exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode(["code" => 201, "msg" => "支付宝返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : "")], JSON_UNESCAPED_UNICODE); exit;
}
