...
$endpoint = 'https://lvse.eu.org/upload/localhost';
...
curl_setopt_array($ch, [
    ...
    CURLOPT_HTTPHEADER => [
        'Origin: https://lvse.eu.org',
        'Referer: https://lvse.eu.org/',
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接绿色图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode(["code" => 201, "msg" => "绿色图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : "")], JSON_UNESCAPED_UNICODE); exit;
}
...
if ($code === 200 && $imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
    ...
    echo json_encode(["code" => 201, "msg" => "绿色图床请求错误（400）：缺少 file 字段。"]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过绿色图床限制（5MB）。"]);
} else {
