...
/**
 * @file api/fatcat.php
 * @description 肥喵图床上传适配器（fatcattech.cn）
 * @author AI
 * @version 1.1.0-dev
 * @date 2026-08-04
 */
...
// 生成 uuid 和 sign（sign 为纯 Unix 时间戳）
$uuid = 'o_' . bin2hex(random_bytes(16)) . substr(md5(uniqid()), 0, 8);
$sign = (string)time();

$endpoint = 'https://image.fatcattech.cn/app/upload.php';
...
curl_setopt_array($ch, [
    ...
    CURLOPT_HTTPHEADER => [
        'Origin: https://image.fatcattech.cn',
        'Referer: https://image.fatcattech.cn/',
        'Accept: */*',
    ],
]);
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接肥喵图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode(["code" => 201, "msg" => "肥喵图床返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : "")], JSON_UNESCAPED_UNICODE); exit;
}

$result = $imgData['result'] ?? '';
$imgUrl = $imgData['url'] ?? '';

if ($result === 'success' && $imgUrl !== '' && filter_var($imgUrl, FILTER_VALIDATE_URL)) {
    echo json_encode(["code" => 200, "msg" => "上传成功！", "path" => $imgUrl], JSON_UNESCAPED_UNICODE);
} elseif ($httpCode === 400) {
    echo json_encode(["code" => 201, "msg" => "肥喵图床请求错误（400）：缺少必要字段。"]);
} elseif ($httpCode === 413) {
    echo json_encode(["code" => 201, "msg" => "文件大小超过肥喵图床限制。"]);
} else {
    echo json_encode(["code" => 201, "msg" => "上传失败（HTTP " . $httpCode . "）", "debug" => $uploadimg], JSON_UNESCAPED_UNICODE);
}
