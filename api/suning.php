...
$authId = trim($conf['api_suning_authid'] ?? '');
if ($authId === '') {
    echo json_encode(["code" => 201, "msg" => "请先在后台苏宁易购图床设置中填写authId（苏宁Cookie中的authId值）后再上传图片！"]); exit;
}
...
curl_setopt_array($ch, [
    CURLOPT_URL => 'https://ffapi.cn/int/v1/image.suning',
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => [
...
if ($uploadimg === false || $uploadimg === '') {
    echo json_encode(["code" => 201, "msg" => "连接苏宁易购图床失败：" . ($curlErr ?: '空响应') . " [HTTP " . $httpCode . "]"]); exit;
}
...
if (!is_array($imgData)) {
    ...
    echo json_encode(["code" => 201, "msg" => "苏宁易购返回非JSON [HTTP " . $httpCode . "]" . ($snippet ? "：" . $snippet : "")], JSON_UNESCAPED_UNICODE); exit;
}
